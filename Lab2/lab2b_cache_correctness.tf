...
# Explanation: Static files are the easy win—megatron caches them like hyperfuel for speed.
...
# Explanation: APIs are dangerous to cache by accident—megatron disables caching until proven safe.
resource "aws_cloudfront_cache_policy" "megatron_cache_api_disabled01" {
...
# Explanation: Origins need context—megatron forwards what the app needs without polluting the cache key.
resource "aws_cloudfront_origin_request_policy" "megatron_orp_api01" {
...
# Explanation: Static origins need almost nothing—megatron forwards minimal values for maximum cache sanity.
resource "aws_cloudfront_origin_request_policy" "megatron_orp_static01" {
...
# Explanation: Make caching intent explicit—megatron stamps Cache-Control so humans and CDNs agree.
resource "aws_cloudfront_response_headers_policy" "megatron_rsp_static01" {
...
#Below was added to lab2_cf_alb.tf since it already had #default_cache_behavior included.
##############################################################
#6) Patch your CloudFront distribution behaviors
##############################################################

# Explanation: Default behavior is conservative—megatron assumes dynamic until proven static.
