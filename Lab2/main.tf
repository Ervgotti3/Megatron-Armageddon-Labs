...
locals {
  ...
  #waf_log_destination = lower(var.waf_log_destination)
  #waf_log_destination = "s3" # Best practice: Use local variable for clarity. (instead of using var everywhere,)
  waf_log_destination = "firehose" # Best practice: Use local variable for clarity. (instead of using var everywhere,)

  # keeping this local for clarity)
  waf_dest_is_cloudwatch = local.waf_log_destination == "cloudwatch"
  waf_dest_is_s3         = local.waf_log_destination == "s3"
  waf_dest_is_firehose   = local.waf_log_destination == "firehose"
}
...
resource "aws_secretsmanager_secret" "megatron_db_secret01" {
  count = local.waf_dest_is_firehose ? 1 : 0
...
resource "aws_secretsmanager_secret_version" "megatron_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.megatron_db_secret01[0].id
...
resource "aws_cloudwatch_log_group" "megatron_log_group01" {
  count             = local.waf_dest_is_cloudwatch ? 1 : 0
...
resource "aws_s3_bucket" "megatron_alb_logs_bucket01" {
  ...
  # Optional for labs (delete even if logs exist)
  force_destroy = true
...
resource "aws_s3_bucket" "megatron_waf_logs_bucket01" {
  ...
  count         = local.waf_dest_is_s3 ? 1 : 0 # Best practice: Use local variable for clarity. (use your locals everywhere)
